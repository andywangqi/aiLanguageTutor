...
function renderBlock(block: Record<string, unknown>, index: number) {
  ...
  if (type === "feature_cards") {
    ...
    return cards.length ? <div className="blog-feature-cards" key={index}>{cards.map((item, itemIndex) => { const value = item && typeof item === "object" ? item as Record<string, unknown> : { title: String(item) }; return <article className="blog-block-card" key={itemIndex}><h3>{typeof value.title === "string" ? value.title : ""}</h3><p>{typeof value.text === "string" ? value.text : typeof value.description === "string" ? value.description : ""}</p></article>; })}</div> : null;
  }
  if (type === "internal_link" && typeof block.href === "string") return <p className="blog-internal-link" key={index}><Link href={block.href}>{typeof block.label === "string" ? block.label : typeof block.text === "string" ? block.text : typeof block.title === "string" ? block.title : block.href} <ArrowRight size={15} aria-hidden="true" /></Link></p>;
