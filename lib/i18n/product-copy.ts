...
const baseProduct: ProductCopy = {
  ...
  auth: {
    ...
    demoMode: "Demo mode is active. Add Supabase environment variables to enable Google sign-in.",
...
const localized: Record<Exclude<Locale, "en">, ProductCopy> = {
  ja: {
    ...
    auth: { ...baseProduct.auth, ariaLabel: "ログイン", closeLabel: "ログインを閉じる", eyebrow: "AI語学チューター", title: "続けるにはログインしてください", lead: "AIチューターと実際に会話しながら、前回の学習内容も続きから練習できます。", google: "Googleで続ける", googleLoading: "Googleに接続中", accountNote: "Googleログインなら、チューター、会話履歴、学習カードをまとめて保存できます。", demoMode: "デモモードです。Googleログインを有効にするにはSupabaseの環境変数を設定してください。", callbackError: "Googleログインを完了できませんでした。もう一度お試しください。", termsPrefix: "続けることで、", termsLabel: "利用規約", termsAnd: "と", privacyLabel: "プライバシーポリシーに同意したものとみなされます。" },
  ...
  th: {
    ...
    auth: { ...baseProduct.auth, ariaLabel: "เข้าสู่ระบบ", closeLabel: "ปิดหน้าล็อกอิน", eyebrow: "AI LANGUAGE TUTOR", title: "เข้าสู่ระบบเพื่อฝึกต่อ", lead: "ฝึกบทสนทนาจริงกับ AI Tutor และกลับมาเรียนต่อจากความคืบหน้าเดิมได้", google: "ดำเนินการต่อด้วย Google", googleLoading: "กำลังเชื่อมต่อกับ Google", accountNote: "การเข้าสู่ระบบด้วย Google จะเก็บ Tutor ประวัติการสนทนา และการ์ดการเรียนรู้ไว้ด้วยกัน", demoMode: "กำลังอยู่ในโหมดเดโม ตั้งค่าตัวแปรสภาพแวดล้อมของ Supabase เพื่อเปิดใช้ Google Sign-in", callbackError: "เข้าสู่ระบบด้วย Google ไม่สำเร็จ กรุณาลองอีกครั้ง", termsPrefix: "เมื่อดำเนินการต่อ คุณยอมรับ", termsLabel: "ข้อกำหนดการใช้บริการ", termsAnd: "และ", privacyLabel: "นโยบายความเป็นส่วนตัว" },
  ...
  ko: {
    ...
    auth: { ...baseProduct.auth, ariaLabel: "로그인", closeLabel: "로그인 닫기", eyebrow: "AI 언어 튜터", title: "계속하려면 로그인하세요", lead: "AI 튜터와 실제 대화를 연습하고 학습 기록을 이어서 확인하세요.", google: "Google로 계속하기", googleLoading: "Google에 연결 중", accountNote: "Google 로그인으로 Tutor, 대화 기록, 학습 카드를 한곳에 보관할 수 있습니다.", demoMode: "현재 데모 모드입니다. Google 로그인을 사용하려면 Supabase 환경 변수를 설정하세요.", callbackError: "Google 로그인을 완료하지 못했습니다. 다시 시도해주세요.", termsPrefix: "계속하면", termsLabel: "이용약관", termsAnd: "및", privacyLabel: "개인정보 처리방침에 동의하게 됩니다." },
  ...
  "zh-CN": {
    ...
    auth: { ...baseProduct.auth, ariaLabel: "登录", closeLabel: "关闭登录", eyebrow: "AI 语言导师", title: "登录后继续练习", lead: "和 AI Tutor 练习真实对话，登录后可以保留你的会话和学习进度。", google: "使用 Google 继续", googleLoading: "正在连接 Google", accountNote: "使用 Google 登录后，你的 Tutor、会话记录和学习卡片会保存在同一个账号里。", demoMode: "当前是演示模式。配置 Supabase 环境变量后才能启用 Google 登录。", callbackError: "Google 登录没有完成，请重试。", termsPrefix: "继续操作即表示你同意", termsLabel: "服务条款", termsAnd: "和", privacyLabel: "隐私政策。" },
  ...
  "zh-TW": {
    ...
    auth: { ...baseProduct.auth, ariaLabel: "登入", closeLabel: "關閉登入", eyebrow: "AI 語言導師", title: "登入後繼續練習", lead: "和 AI Tutor 練習真實對話，登入後可以保留你的會話和學習進度。", google: "使用 Google 繼續", googleLoading: "正在連線 Google", accountNote: "使用 Google 登入後，你的 Tutor、會話記錄和學習卡片會保存在同一個帳號裡。", demoMode: "目前是展示模式。設定 Supabase 環境變數後才能啟用 Google 登入。", callbackError: "Google 登入沒有完成，請重試。", termsPrefix: "繼續操作即表示你同意", termsLabel: "服務條款", termsAnd: "和", privacyLabel: "隱私政策。" },
